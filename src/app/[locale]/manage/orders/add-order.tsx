...
export default function AddOrder() {
  ...
  const handleNewGuestOrder = form.handleSubmit(async () => {
    const guestRes = await createGuestMutation
      .mutateAsync({
        name,
        tableNumber
      })
  ...
  const handleOrder = (event?: React.BaseSyntheticEvent) => {
    if (isNewGuest) {
      return handleNewGuestOrder(event)
    }
    return handleOldGuestOrder()
  }
